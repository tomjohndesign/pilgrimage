...
for (const [type, entry] of Object.entries(pack.callings)) {
  ...
  for (const clip of ["walk", "idle"]) {
    const { data, info } = await sharp(`public${entry[clip]}`).ensureAlpha().raw().toBuffer({ resolveWithObject: true })
...
for (const clip of ["walk", "idle"]) {
  const { data, info } = await sharp(`public${pack.shadows[clip]}`).ensureAlpha().raw().toBuffer({ resolveWithObject: true })
  if (info.width !== (clip === "walk" ? 8 : 1) * size || info.height !== pack.rows * size) throw new Error("Shadow dimensions do not match bodies")
