...
import { claimTownBuildings, settlementMap, createSettlement, purchaseStructure, demolishStructure, creditTimber, creditAdmission, creditTrade, payWages, syncWages, syncTimberSpending, settlementRenown, grantResources, grantRenown, completeConstruction, type Resources } from "@/lib/game/settlement"
  ...
  const map = useMemo(
    () =>
      world
        ...
        : null,
    [world, session.settlement.elevation, session.settlement.structures, session.settlement.claimedBuildings, session.settlement.demolishedBuildings],
  )
