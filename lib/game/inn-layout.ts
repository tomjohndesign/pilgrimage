...
export function innLayout(width: number, depth: number, upper = false, flue?: {x:number;z:number}) {
  ...
  // Include the wooden frame beyond the mattress and leave the plaster clear.
  // Upstairs walls project beyond the host, leaving room for the ladder aisle.
  const sideBedX=width/2-(upper ? .36 : .22+bedWidth/2+.025)
  const beds=([-1,0,1] as const).flatMap(side=>Array.from({length:rows},(_,row)=>({
    x:side*sideBedX,z:-depth/2+.12+(row+.5)*step,
