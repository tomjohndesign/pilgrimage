...
export const PERSON_CLIPS = {
  ...
  sitting: { label: "Sitting · ground", frames: 8 },
  sittingChair: { label: "Sitting · chair", frames: 8 },
...
export const ACTION_CLIPS = ["wearyWalk", "sleeping", "sitting", "sittingChair", "seatedMeal", "seatedDrink", "seatedPrayer", "praying", "treeFelling", "woodcutting", "building", "gathering", "carrying", "hoisting", "procession", "preaching", "drinking", "drinkingLow"] as const
...
export function pelvisHeight(phase: number, clip: BaseClip, b = BASE_PERSON.body): number {
  ...
  if (clip === "sittingChair" || clip === "seatedPrayer" || clip === "seatedMeal" || clip === "seatedDrink") return b.ankleHeight + b.shinLength
...
export function legPose(side: BodySide, phase: number, clip: BaseClip, b = BASE_PERSON.body): LegPose {
  if (clip === "sittingChair" || clip === "seatedPrayer" || clip === "seatedMeal" || clip === "seatedDrink") {
    const x = (side === "left" ? 1 : -1) * b.legOffset
