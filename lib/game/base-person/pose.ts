...
export const PERSON_CLIPS = {
  ...
  sleeping: { label: "Sleeping", frames: 8 },
  sitting: { label: "Sitting", frames: 8 },
  praying: { label: "Praying", frames: 8 },
  woodcutting: { label: "Woodcutting", frames: 8 },
  gathering: { label: "Gathering", frames: 8 },
...
export function legPose(side: BodySide, phase: number, clip: BaseClip, b = BASE_PERSON.body): LegPose {
  if (clip === "sitting" || clip === "praying") {
    ...
    const kneeling = clip === "praying"
    const hip: Point3 = [x, kneeling ? 0.1 + b.thighLength * 0.9 : 0.25, 0]
  ...
  const hip: Point3 = [x, b.hipHeight - (clip === "gathering" ? 0.24 : 0), 0]
