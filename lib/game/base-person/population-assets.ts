...
import manifest from "../../../public/textures/characters/population/v20/manifest.json"
...
export function populationVisual(type: TravelerTypeId, variant: number, custom: PopulationPack | null, age = 18) {
  ...
  return {
    ...
    actions: Object.fromEntries(ACTION_CLIPS.flatMap(clip => {
      ...
      return url && shadow ? [[clip, { url, shadow, playbackRate: actionPlaybackRate(clip, calling.designs[variant]), columns: pack.frameCounts?.[clip] ?? pack.actionFrames?.[clip] ?? 8, rows: pack.rows, stillFrame: 0 }]] : []
