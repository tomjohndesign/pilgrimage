...
export const DEFAULT_DESIGN: PersonDesign = {
  bodyType: "Male", head: 1.2, build: 1, torsoHeight: 1, shoulderHeight: 1.15, neckHeight: 0.65, tunicLength: 1.4,
  legs: 0.9, feet: 1, footWidth: 0.95, footHeight: 0.7, hem: 1.25, stride: 0.8, ink: 0.6,
...
export const PERSON_PRESETS: Record<string, PersonDesign> = {
  ...
  Female: { ...DEFAULT_DESIGN, bodyType: "Female", hairStyle: "Long", beard: false },
...
const legacyDefaults: Partial<Record<DesignKey, number>> = { torsoHeight: 1, footWidth: 1, footHeight: 1, tunicLength: 1, shoulderHeight: 1, neckHeight: 1, nose: 1, shadow: 0.16 }
export function validatePersonDesign(input: unknown): PersonDesign {
  ...
  for (const key of ["tunicColor", "skinColor", "hairColor"] as const) {
...
export function personRecipe(input: PersonDesign = DEFAULT_DESIGN) {
  ...
  result.renderPalette = [...new Set(["#30251e", ...[0.5, 0.7, 0.9, 1.1, 1.3].map(f => shade(design.skinColor, f)),
    ...[0.55, 0.8, 1, 1.3].map(f => shade(design.tunicColor, f)), ...[0.65, 1, 1.4].map(f => shade(design.hairColor, f))])]
  ...
  // Length moves only the skirt hem, leaving the belt and upper torso fixed.
  const tunicDrop = (original.shoulderHeight + 0.01 - (original.hipHeight - 0.11)) * (design.tunicLength - 1)
  const tunicHem = b.hipHeight - 0.11 - tunicDrop
  const tunicHemUpper = b.hipHeight - 0.07 - tunicDrop
