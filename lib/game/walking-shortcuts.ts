 ...
 * it and the traffic has to find its own way past. The obstruction is cached
 * per map, keyed on the building list it was measured from.
 */
const roadObstructions = new WeakMap<GameMap, { buildings: readonly BuildingDef[]; blocked: Uint8Array }>()
export function blockedRoad(map: GameMap): Uint8Array {
  const road = map.road ?? []
  const cached = roadObstructions.get(map)
  if (cached && cached.buildings === map.buildings) return cached.blocked
  const blocked = new Uint8Array(road.length)
  for (let i = 0; i < road.length; i++) {
    const p = road[i]
    blocked[i] = map.buildings.some(b => p.x >= b.x && p.x < b.x + b.w && p.z >= b.z && p.z < b.z + b.d) ? 1 : 0
  }
  roadObstructions.set(map, { buildings: map.buildings, blocked })
  return blocked
}
