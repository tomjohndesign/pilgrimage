...
import { distanceToRoadSegments, roadSegmentWear, traveledRoadSegments } from "./road-segments"
...
describe("faded cart corner wear", () => {
  it("keeps cart ruts visible alongside traffic that has worn away", () => {
    const map = turningMap("right")
    const cart = [...cartCornerWear(map).values()].flat()[0]
    const local = [...traveledRoadSegments(map, [{ ax: 2, az: 2, bx: 3, bz: 2, wear: 0.1 }]).values()].flat()[0]
    expect(roadSegmentWear(cart, 0, 0, 0)[3]).toBe(1)
    expect(roadSegmentWear(local, 0, 0, 0)[3]).toBe(0.5)
    expect(roadSegmentWear([local[0], local[1], local[2], local[3], local[4], 0], 0, 0, 0)[3]).toBe(0)
    expect(cart[4]).not.toBe(local[4])
  })
  it.each(["right", "left", "s_bend"] as const)("follows the inside cart shortcut for %s without changing walking lanes", scenario => {
    ...
    expect([...bins.values()].flat().every(s => s[4] === 3)).toBe(true)
