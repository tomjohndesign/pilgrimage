...
import { LANDING_CHAPEL, LANDING_ROUTE_LENGTH, LANDING_TERRAIN, LANDING_TILES, landingMonkPoint } from "./landing-layout"
...
describe("landing chapel paths", () => {
  it("keeps the full monk route on drawn path tiles and outside the chapel walls", () => {
    ...
    for (let distance = 0; distance < LANDING_ROUTE_LENGTH; distance += .025) {
      ...
      expect(Math.abs(point.x) >= LANDING_CHAPEL.w / 2 || Math.abs(point.z) >= LANDING_CHAPEL.d / 2).toBe(true)
    }
  })
