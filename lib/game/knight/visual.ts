import manifest from "../../../public/textures/knights/v6/manifest.json"
...
export function knightVisual(variant: number) {
  ...
  const clip = (name: keyof typeof manifest.person.frameCounts) => ({ url: `/textures/knights/${KNIGHT.version}/knight-${name}.png`, columns: manifest.person.frameCounts[name], rows: manifest.person.rows, stillFrame: 0 })
...
export function squireVisual() {
  ...
  const clip = (name: keyof typeof data.frameCounts) => ({ url: `/textures/knights/${KNIGHT.version}/squire-${name}.png`, columns: data.frameCounts[name], rows: data.rows, stillFrame: 0 })
