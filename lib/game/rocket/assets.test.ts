...
import manifest from "../../../public/textures/characters/rockets/v13/manifest.json"
...
describe("rocket monk assets", () => {
  ...
  it.each(["brown", "grey"])("ships complete, registered %s sheets with binary alpha and safe margins", async hair => {
    for (const [clip, columns] of Object.entries(manifest.frameCounts)) {
      const { data, info } = await sharp(`public/textures/characters/rockets/v13/${hair}-${clip}.png`).ensureAlpha().raw().toBuffer({ resolveWithObject: true })
