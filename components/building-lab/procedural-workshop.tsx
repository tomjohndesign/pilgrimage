...
export function ProceduralWorkshop({ mode, onModeChange, active: workspaceActive = true }: AssetEditorNavigation & { active?: boolean }) {
  ...
  function showChickenCoopExample() {
    setRecipe({...earlyBuildingRecipe("chicken-coop"),view:2})
    setSelectedId(null);setPlacement(null)
    setNeighbors([{id:"egg-store",recipe:earlyBuildingRecipe("storehouse"),x:3,z:0,rotation:0}]);setNeighbor(true)
    setNotice("Hens enter through the pop-hole to lay. The keeper opens the rear hatch, collects eggs and carries them to the raised store. Select the coop to see the nests.")
  }
  ...
  const [eggCounts,setEggCounts]=useState({waiting:0,stored:0})
  ...
  return <AssetEditorFrame onSelectionChange={setEntityActive} mode={mode} onModeChange={onModeChange} label="Procedural building editor"
    ...
    status={<>{placement ? placementStatus : selectedId ? `${selectedNeighbor?.recipe.subject ?? recipe.subject} selected · click again or the ground to deselect` : notice || "Live procedural model · settings save in this browser"}{recipe.variant === "chicken-coop" && <span> · Eggs: {eggCounts.waiting} in nests · {eggCounts.stored} stored</span>}</>} detail={`${recipe.width} × ${recipe.depth} tiles · 4 directions`}>
    <AssetEditorWorkspace title="Building" controlsOpen={controlsOpen} onControlsClose={() => setControlsOpen(false)}
      ...
      controls={<><AssetEditorSection title="Shape"><label className="person-choice">Form<EntitySelect aria-label="Building form" value={recipe.variant} staging={{ kind: "buildings", active: workspaceActive }} onChange={event => {
          ...
          if (!selected) return
          if (selected.id === "chicken-coop") { showChickenCoopExample(); return }
...
<AssetEditorSection title="Shape">
            ...
            {!["enclosure","garden","cross","lumberCamp"].includes(recipe.variant) && <Tuner label="Roof rise" labelClassName="w-28" value={recipe.roofRise} min={recipe.variant === "chicken-coop" ? 0 : .2} max={1.5} step={0.025} display={`${recipe.roofRise.toFixed(2)} tiles`} onChange={value => update("roofRise", value)} />}
...
<AssetEditorSection title="Scene">
            ...
            <ChromeButton className="hud-action" onClick={showChickenCoopExample}>Chicken coop + egg collection</ChromeButton>
...
<AssetEditorSection title="Files">
      ...
      toolbar={<><div className="person-playback"><span className="person-hint">{AVAILABLE_EARLY_BUILDINGS.find(v => v.id === recipe.variant)?.name}{neighbor && neighbors.length ? ` + ${neighbors.length} buildings` : ""}</span>{["sheep-pen","chicken-coop"].includes(recipe.variant) && <label>Speed<ChromeSelect aria-label="Flock demo speed" value={playbackRate} onChange={e=>setPlaybackRate(Number(e.target.value))}>{[1,2,3,6].map(rate=><option key={rate} value={rate}>{rate}×</option>)}</ChromeSelect></label>}</div>
<div className="person-view-buttons" aria-label="Building preview modes">{recipe.variant === "chicken-coop" && <ChromeButton className="hud-action" onClick={showChickenCoopExample}>Replay egg collection</ChromeButton>}{recipe.variant === "sheep-pen" && <><ChromeButton className="hud-action" onClick={()=>{setSlaughterDemo(n=>-Math.abs(n)-1);setNotice("Sheep and goats are milked beside the flock. Workers carry milk in buckets to the public food platform.")}}>Show milking cycle</ChromeButton><ChromeButton className="hud-action" onClick={()=>{setSlaughterDemo(n=>Math.abs(n)+1);setNotice("The shepherd leads an animal to the work spot, collects three small trays of meat onto the public food platform, then replenishes the flock. Each tray adds 50 meat; milk arrives in buckets between harvests.")}}>Show slaughter cycle</ChromeButton></>}<ChromeSelect aria-label="Building view" value={allViews ? "all" : "map"} onChange={e => setAllViews(e.target.value === "all")}><option value="map">On the map</option><option value="all">All four</option></ChromeSelect></div></>}
