...
export function TerrainTiles({
  ...
  const roadCount = useMemo(
    () => map.tiles.reduce((n, t, i) => ((isRoadTerrain(t) || shoulders.has(i) || diagonalSegments.has(i) || floors.tiles.has(i)) && !coveredLand.has(i) ? n + 1 : n), 0),
    [map, coveredLand, shoulders, diagonalSegments, floors],
  )
  ...
  useMemo(() => {
    for (const texture of [roadTexture, trailTexture, grass]) {
      texture.colorSpace = THREE.SRGBColorSpace
      texture.wrapS = THREE.RepeatWrapping
      texture.wrapT = THREE.RepeatWrapping
      texture.anisotropy = 4
    }
  }, [roadTexture, trailTexture, grass])
  ...
  useLayoutEffect(() => {
    ...
    for (let z = 0; z < map.depth; z++) {
      for (let x = 0; x < map.width; x++) {
        ...
        if (isRoadTerrain(terrain) || shoulders.has(index) || diagonalSegments.has(index) || (floors.tiles.has(index) && !coveredLand.has(index))) {
        ...
        color.copy(land.color).multiplyScalar(jitter * canopy)
        {
          ...
          const shoreCorners = coveredLand.has(index) ? [0, 0, 0, 0] as const : shorelineCorners(map, x, z)
