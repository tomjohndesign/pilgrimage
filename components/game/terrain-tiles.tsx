...
import { CHARACTER_PIXEL_SIZE, CHARACTER_PIXELS_PER_UNIT } from "@/lib/game/render/pixel-scale"
...
/**
 * Generated by scripts/generate-dirt-texture.mjs: topsoil over packed earth
 * over bedded stone, one texel per character pixel, measured down from the
 * local rim. Keep the size in sync with that script.
 */
const DIRT_TEXTURE_URL = "/textures/dirt-side.png"
const CLIFF_TEXTURE = { width: 512, height: 256 }
/**
 * World-space UVs for every cliff face: along the face at the character pixel
 * density (cut corners measure along their diagonal) and down from the rim,
 * so a figure and the rock beside it share one pixel size and every bank
 * starts with its turf and humus on top.
 */
const CLIFF_UV_GLSL = `
  vec2 cliffFaceUv(vec2 worldXZ, vec3 faceNormal, float depthBelowRim) {
    vec2 tangent = vec2(faceNormal.z, -faceNormal.x);
    float span = length(tangent);
    tangent = span > 0.001 ? tangent / span : vec2(1.0, 0.0);
    return vec2(dot(worldXZ, tangent) * ${(CHARACTER_PIXELS_PER_UNIT / CLIFF_TEXTURE.width).toFixed(6)},
      1.0 - depthBelowRim * ${(CHARACTER_PIXELS_PER_UNIT / CLIFF_TEXTURE.height).toFixed(6)});
  }`
...
/**
 * The slab's sides continue the cliff below the edge tiles. Whatever the edge
 * tiles' own faces show, the slab starts inside the bedrock — the texture's
 * soil never reaches this depth and its rock repeats vertically — so the stack
 * reads as one bank of soil over rock that keeps going, never a second bank.
 * Keep in sync with ROCK_TOP in scripts/generate-dirt-texture.mjs.
 */
const SLAB_ROCK_DEPTH = 1.2
function makeSlabMaterial(cliffTexture: THREE.Texture): THREE.MeshLambertMaterial {
  const material = new THREE.MeshLambertMaterial()
  material.onBeforeCompile = (shader) => {
    shader.uniforms.cliffMap = { value: cliffTexture }
    shader.vertexShader = shader.vertexShader
      .replace("#include <common>", `varying vec2 vCliffUv;
        ${CLIFF_UV_GLSL}
        #include <common>`)
      .replace("#include <project_vertex>", `#include <project_vertex>
        vCliffUv = cliffFaceUv((modelMatrix * vec4(transformed, 1.0)).xz, objectNormal,
          ${SLAB_THICKNESS / 2} - transformed.y + ${SLAB_ROCK_DEPTH});`)
    shader.fragmentShader = shader.fragmentShader
      .replace("#include <common>", `varying vec2 vCliffUv;
        uniform sampler2D cliffMap;
        #include <common>`)
      .replace("#include <map_fragment>", "diffuseColor.rgb *= texture2D(cliffMap, vCliffUv).rgb;")
  }
  return material
}
    ...
    shader.vertexShader = shader.vertexShader
      ...
      .replace(
        ...
        `#include <project_vertex>
        {
          ...
          #endif
          vec4 worldPos = modelMatrix * terrainPos;
          vWorld = worldPos.xz;
          vWater = mod(aSurface.y, 2.0);
          // The rim is in terrain space, like terrainTop; a lifted preview must not sink the strata.
          vCliffUv = cliffFaceUv(worldPos.xz, objectNormal, terrainTop - terrainPos.y);
...
export function TerrainTiles(props: TerrainProps) {
  ...
  const waterPalette = useMemo(() => {
    ...
    for (let i = 0; i < map.tiles.length; i++) {
      ...
      let shore = shorelineCorners(map, i % map.width, Math.floor(i / map.width)).reduce((bits, flag, c) => bits + flag * 2 ** c, 0)
      // A bank cut back to the water is wet on its shelf half, so neighbouring
      // water sampling across the edge finds water there, not a fringe of turf.
      const cut = cuts.get(i)
      if (cut && !wet && map.tiles[cut.donor] === "water" && cliffCorner(map, i % map.width, Math.floor(i / map.width))) shore |= 2 ** cut.corner
      data.set([paint.color.r, paint.color.g, paint.color.b, corners[i] + 1 + (wet ? 8 : 0) + shore * 16], i * 4)
    }
    const texture = new THREE.DataTexture(data, map.width, map.depth, THREE.RGBAFormat, THREE.FloatType)
    texture.needsUpdate = true
    return texture
  }, [map.tiles, map.width, map.depth, map.seed, map.water, cuts])
  ...
  useMemo(() => {
    ...
    // Repeat along the cliff, but run topsoil to bedrock just once from the
    // rim down, so wrap horizontally and clamp vertically. Nearest sampling
    // keeps its texels the size of character pixels, like every ground sprite.
    dirt.wrapS = THREE.RepeatWrapping
    dirt.wrapT = THREE.ClampToEdgeWrapping
    dirt.magFilter = THREE.NearestFilter
    dirt.minFilter = THREE.NearestMipmapLinearFilter
  }, [dirt])
  const slabMaterial = useMemo(() => makeSlabMaterial(dirt), [dirt])
  ...
  return <group name="terrain">
    {props.slab !== false && <>
      <mesh position={slabPosition} material={slabMaterial}><boxGeometry args={slabArgs} /></mesh>
    ...
    for (let z = bounds.z; z < bounds.endZ; z++) {
      for (let x = bounds.x; x < bounds.endX; x++) {
        ...
        color.copy(land.color).multiplyScalar(jitter)
        {
          ...
          // The upper half of a cliff cut paints its own record only: stippled
          // sampling across the diagonal would sprinkle the shelf's water on it.
          // Shoreline halves keep the shared sampling that blends depth bands.
          target.surface.setZ(i, cut && cliffCorner(map, x, z) ? index + 1 : 0)
