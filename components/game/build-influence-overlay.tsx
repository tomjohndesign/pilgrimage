...
/**
 * Show ground availability while building, without a renown boundary.
 ...
 */
export function BuildInfluenceOverlay({ map, buildMode }: { map: GameMap; buildMode: boolean; balance?: GameBalance }) {
  const geometry = useMemo(() => {
    const positions: number[] = [], colors: number[] = [], churchEdges: number[] = []
    ...
    return { tiles, churchBoundary }
  }, [map, buildMode])
  useEffect(() => () => { geometry.tiles.dispose(); geometry.churchBoundary.dispose() }, [geometry])
  return (
    <group>
