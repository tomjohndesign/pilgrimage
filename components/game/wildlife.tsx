...
export function WildlifeBatch({ kind, animals, map, scale, grazing }: { kind: WildlifeKind; animals: WildlifeAnimal[]; map: GameMap; scale: number; grazing?: number }) {
  ...
  useFrame(({ camera }) => {
    ...
    for (let i = 0; i < animals.length; i++) {
      ...
      rig.pose(posePhase, animal.moving, animal.age, graze, wingBlend, animal.gait, { burrow, glide, drive: animal.drive, lying: animal.lying, edits: useAnimalRigStore.getState().designs[kind], clip: bird ? flight ? glide > .5 ? "glide" : "fly" : "idle" : animal.moving ? animal.gait : animal.action })
  ...
  return <group name={`wildlife-${kind}`}>
    <mesh geometry={batch.geometry} material={material} frustumCulled={false} onClick={event => {
      ...
      const animal = animals[Math.floor(event.faceIndex / batch.trianglesPerAnimal)]
