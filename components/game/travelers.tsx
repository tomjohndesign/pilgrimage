...
export function Travelers({
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < travelers.length; i++) {
      ...
      group.userData.pastureY = s.pasture && map.elevation ? groundHeight(map, s.pasture.x + map.width / 2 - 0.5, s.pasture.z + map.depth / 2 - 0.5) : s.y
    ...
    <group>
      {travelers.map((traveler, index) => {
        ...
        return (
          ...
          >
            <TravelerFigure {...cartLoadout(traveler.id)} appearance={appearances[index]} selected={selected} type={traveler.type} onClick={select} idColor={idColor}
