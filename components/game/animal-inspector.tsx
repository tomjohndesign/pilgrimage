...
export function AnimalInspector({ id }: { id: number }) {
  ...
  return <div className="hud-inspector-content w-[250px] border border-rule bg-parchment/95 px-4 py-3 text-[11px] text-ink">
    <div className="mb-2 flex items-center justify-between gap-3"><span className="font-display text-xs">{WILDLIFE_PROFILES[animal.kind].label}</span><div className="flex items-center gap-2"><FollowButton subject="animal" /><ChromeButton aria-label="Dismiss animal" onClick={() => useCameraStore.getState().select(null)}>✕</ChromeButton></div></div>
