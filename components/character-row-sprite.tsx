...
type Sprite = { idle: HTMLCanvasElement; walk: HTMLCanvasElement; size: number; frames: number; fps: number; bounds: [number, number, number, number] }
...
function bounds(idle: HTMLCanvasElement, walk: HTMLCanvasElement, size: number): Sprite["bounds"] {
...
async function loadSprite(id: string, draft?: PersonDesign, sequence: BaseClip = "walk"): Promise<Sprite> {
  ...
  if (preset) {
    ...
    if (id === "cart") {
      ...
      size = CART.cellSize; frames = TRANSPORT.wheelFrames; fps = 12; row = 2
      ...
      row = variant * 8 + 1; size = knightMetadata.person.cellSize; frames = knightMetadata.person.frameCounts[sequence as keyof typeof knightMetadata.person.frameCounts] ?? knightMetadata.person.frameCounts.walk
  ...
  return { idle, walk, frames, size, fps, bounds: bounds(idle!, walk!, size) }
}
function sprite(id: string, design?: PersonDesign, sequence: BaseClip = "walk") {
...
export function CharacterRowSprite({ id, active, design, sequence = "walk", size = 16 }: { id: string; active: boolean; design?: PersonDesign; sequence?: "random" | "walk"; size?: number }) {
  ...
  const [idleSource, setIdleSource] = useState<Sprite | null>(null)
  const [movingSource, setMovingSource] = useState<Sprite | null>(null)
  ...
  useEffect(() => { if (!visible) return; let cancelled = false
    sprite(id, design, "idle").then(result => { if (!cancelled) setIdleSource(result) }).catch(error => console.warn(`Character preview ${id}:`, error))
    return () => { cancelled = true }
  }, [id, visible, design])
  useEffect(() => { if (!visible || !active || reducedMotion || !idleSource) return; let cancelled = false
    sprite(id, design, clip).then(result => { if (!cancelled) setMovingSource(result) }).catch(error => console.warn(`Character animation ${id}:`, error))
    return () => { cancelled = true }
