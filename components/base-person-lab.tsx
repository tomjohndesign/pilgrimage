...
function KnightEntourage({ mounted, row, frame, visibleFrame, variant, walking, zoom = 1, ...tile }: {
  mounted: boolean; row: number; frame: number; variant: number; walking: boolean; zoom?: number
  ...
  return <div className="person-sprite knight-entourage" style={{ width: layout.width * zoom, height: layout.height * zoom }}>
...
export function BasePersonLab({ mode, onModeChange, active = true }: AssetEditorNavigation & { active?: boolean }) {
  ...
  const entourage = isKnight && showSquire ? BASE_PERSON.directions.map((_, i) => entourageLayout(mountedKnight, i)) : null
  const previewWidth = entourage ? Math.max(...entourage.map(layout => layout.width)) : pixels
  const previewHeight = entourage ? Math.max(...entourage.map(layout => layout.height)) : pixels
  const fittedZoom = Math.min(zoom, Math.max(1, Math.floor(Math.min((stageSize.width - 32) / previewWidth, (stageSize.height - 32) / previewHeight))))
    ...
    <div className="person-workspace">
      ...
      <div className="person-preview" aria-label="Character preview">
        ...
        <div className="person-stage-layout"><div ref={stageRef} className={`person-stage person-stage-${view}${scrubbingViews ? " is-scrubbing" : ""}`}
          ...
          </div> : isKnight && showSquire ? <KnightEntourage mounted={mountedKnight} row={row} frame={frame} variant={knightVariant} walking={knightClip === "walk"} url={url} visibleFrame={visibleFrame} columns={columns} cellSize={pixels} rows={atlasRows} zoom={fittedZoom} name={`Knight ${direction}, frame ${step + 1}`} /> : <div className="person-sprite" style={{ width: pixels * fittedZoom, height: pixels * fittedZoom }}>
