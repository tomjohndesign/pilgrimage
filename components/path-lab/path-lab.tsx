...
import { LabPlayback, LabSeedInput, LabSeedActions, LabSelect, LabSlider, labButton as button } from "@/components/lab-controls"
...
export function PathLab({ mode, onModeChange, active = true }: AssetEditorNavigation & { active?: boolean }) {
  ...
  const live = useRef({ experiment, settings, playing, speed, active })
  live.current = { experiment, settings, playing, speed, active }
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const { experiment, settings, playing, speed, active } = live.current
      if (active && playing && !document.hidden) worlds.current![experiment].forEach(w => advanceWorld(w, dt * speed, settings))
      if (active && now - lastLabel > 150) { refresh(n => n + 1); lastLabel = now }
  ...
  const share = async () => {
    ...
    const url = `${window.location.origin}${playgroundHref("paths", params)}`
    ...
    <AssetEditorWorkspace title="Paths" controlsOpen={controlsOpen} onControlsClose={() => setControlsOpen(false)}
      controls={<><AssetEditorSection title="Scenario"><LabSelect label="Scenario" value={experiment} options={Object.fromEntries(Object.entries(EXPERIMENTS).map(([id, item]) => [id, item.label]))} onChange={value => { setExperiment(value as Experiment); setTool("inspect"); setSelected(null); setMessage(""); setShareUrl("") }} /></AssetEditorSection>
        <AssetEditorSection title="Layout"><div className="person-file-actions">
          ...
          <LabSeedInput value={seedDraft} onChange={setSeedDraft} onApply={seed => changePermutation({ seed })} />
          ...
          <div className="flex flex-wrap gap-2">
            <LabSeedActions draft={seedDraft} seed={permutation.seed} onApply={seed => changePermutation({ seed })} />
          </div>
        </div>
